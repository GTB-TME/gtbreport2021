...
p <- ggplot(data=DO,aes(x=M14,y=median)) +
    ...
    geom_point() +
    xlab('prior 2019 incidence') +
    ylab('prior 2020 incidence')
p2 <- p + scale_x_continuous(label=comma) +
    scale_y_continuous(label=comma) +
    coord_fixed(ratio=1,xlim=c(0,250e3),ylim=c(0,250e3))
## p2
ggsave(p2,file=here('disaggregation/R/02children/plots/2019CountryCompare.pdf'))

p3 <- p2 + geom_text_repel(aes(x=M14,y=median,label=iso3),
                           hjust=0,vjust=0) +
    scale_x_sqrt(label=comma) + scale_y_sqrt(label=comma) +
    coord_fixed(ratio=1,xlim=c(0,250e3),ylim=c(0,250e3))
ggsave(p3,file=here('disaggregation/R/02children/plots/2019CountryCompareLabeled.pdf'))

p3 <- p + scale_x_log10()+ scale_y_log10()
## p3
ggsave(p3,file=here('disaggregation/R/02children/plots/2018CountryCompareLog.pdf'))
...
## p4
ggsave(p4,
       file=here('disaggregation/R/02children/plots/2019CountryCompareSqrt.pdf'),
       height=10,width=10)
