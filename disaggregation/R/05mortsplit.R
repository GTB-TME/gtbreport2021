...
xlfn <- here('input/VR/info_TB_may21.xls')
...
if(!file.exists(mfn)){
    ...
    ## -------- ICD>10.1
    M1 <- as.data.table(read_excel(xlfn,sheet = 4,
                                   skip=1,col_types=typz)) #ICD > 10.1
    M1 <- refrm(M1)
    ## -------- ICD 9
    M2 <- as.data.table(read_excel(xlfn,sheet = 5,
                                   skip=1,col_types=typz)) #ICD = 9
    M2 <- refrm(M2)
    ## -------- ICD 8
    M3 <- as.data.table(read_excel(xlfn,sheet = 6,
                                   skip=1,col_types=typz)) #ICD = 8
    M3 <- refrm(M3)
    ## -------- ICD 10.1
    M4 <- as.data.table(read_excel(xlfn,sheet = 7,
                                   skip=1,col_types=typz)) #ICD = 10.1
    M4 <- refrm(M4)
    ...
    ## sub-countries
    VR[name %in% c("French Guiana","Martinique","Reunion","Mayotte"),
       name:="France"]
