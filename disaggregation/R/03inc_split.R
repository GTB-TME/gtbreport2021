...
#' Record out:
(knotem19 <- tb[year==estyr & !is.na(newrel.m014) & is.na(newrel.m04),
                unique(iso3)])
(knotem18 <- tb[year==estyr-1 & !is.na(newrel.m014) & is.na(newrel.m04),
                unique(iso3)])

cat(knotem19,file=here('disaggregation/output/incsplits/data/knotem19.txt'))
cat(knotem18,file=here('disaggregation/output/incsplits/data/knotem18.txt'))
...
#' Record out:
(knotef19 <- tb[year==estyr & !is.na(newrel.f014) & is.na(newrel.f04),unique(iso3)])
(knotef18 <- tb[year==estyr-1 & !is.na(newrel.f014) & is.na(newrel.f04),unique(iso3)])

cat(knotef19,file=here('disaggregation/output/incsplits/data/knotef19.txt'))
cat(knotef18,file=here('disaggregation/output/incsplits/data/knotef18.txt'))
...
est[year==estyr & source.inc=="Predicted incidence ratio",iso3] #COVID disruption, directly
...
for(cn in isopass1){
  ...
  if(plotting){
    fn <- here('disaggregation/output/incsplits/plots/pass1') + glue('/{cn}1.pdf')
    if(gypt){
...
for(cn in isotodo){
  ...
  if(est[iso3==cn,g.hbc[1]]) hi30[[cn]] <- plt #add to hibc30
  if(plotting){
    fn <- here('disaggregation/output/incsplits/plots/pass2') + glue('/{cn}2.pdf')
    if(gypt){
...
gmb <- niplot(tmp3)
ggsave(gmb,device = cairo_pdf,w=W,h=H,
       filename=here('disaggregation/output/incsplits/plots/pass2/GMB2b.pdf'))
...
## cno <- priorsample('BGR',adultadjust = TRUE)
## niplot(cno)
...
for(cn in isopass3){
  ...
  if(est[iso3==cn,g.hbc[1]]) hi30[[cn]] <- plt #add to hibc30
  if(plotting){
    fn <- here('disaggregation/output/incsplits/plots/pass3') + glue('/{cn}3.pdf')
    if(gypt){
      ggsave(filename=fn,plot=plt,device = cairo_pdf,w=W,h=H)
    } else
      ggsave(filename=fn,plot=plt,w=W,h=H)
...
whoz <- levels(regsplt$g.whoregion)
