...
# load(here('data/hrg.rda'))
# load(here('data/ghr.rda'))
...
load(here('report/Olga/DataFig231_ 2021-08-17 .rda'))  # rn9
load(here('report/Olga/MasterDataFig235_6_7_ 2021-08-17 .rda')) # Data_Figs_5_6_7_d
load(here('report/Olga/DataFig232_ 2021-08-17 .rda'))  # rp9
load(here('report/Olga/dataFig237_ 2021-08-17 .rda'))  # dataFig237
load(here('report/Olga/Global_Prop_FQ_Resistance_ 2021-08-17 .rda')) # res4


ind <- fread('input/inc/ind.csv')
model <- fread('csv/model_2021-07-27.csv')
top10 <- fread(here('input/VR/top10.csv'))
rrinc <- fread(here('csv/RRincidence_2021-08-16.csv'))
...
regional[, mort.milestone := mort.num[16] * 0.65, by = g.whoregion]
regional[, inc.milestone := inc[16] * 0.8, by = g.whoregion]
...
hest[, inc.milestone := inc[16] * 0.8, by = iso3]
hest[, mort.milestone := mort.num[16] * 0.65, by = iso3]

est[, inc.milestone := inc[16] * 0.8, by = iso3]
est[, mort.milestone := mort.num[16] * 0.65, by = iso3]
...
save(yr,
     ...
     res4,
     conf,
     rn9, rp9, Data_Figs_5_6_7_d, dataFig237,
     # trenddr,
     unaids,
     global.mort.rr,
     reg.mort.rr,
