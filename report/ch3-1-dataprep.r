# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Data preparation script for ch3-1-txt.rmd
# and ch3-1-figs.rmd
...
t3.1.1_txt <- filter(notification, year >= (report_year - 2)) %>%
  select(iso3,
         year,
         conf_rrmdr,
         conf_rr_nfqr,
         ...
         conf_rr_fqr_tx) %>%
  group_by(year) %>%
  summarise(across(conf_rrmdr:conf_rr_fqr_tx, sum, na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(conf_rrmdr_p = lag(conf_rrmdr)) %>%
  filter(year==report_year-1) %>%
  mutate(detected = conf_rr_nfqr + conf_rr_fqr,
         pct_diff = abs(conf_rr_nfqr + conf_rr_fqr - conf_rrmdr_p)*100/conf_rrmdr_p,
         enrolled = unconf_rr_nfqr_tx + conf_rr_nfqr_tx + conf_rr_fqr_tx) %>%
  #restrict to the variables needed in the text
  select(detected, conf_rr_nfqr, conf_rr_fqr, pct_diff, conf_rrmdr_p, enrolled)
...
f3.1.1_txt <- filter(inc_global, year>=report_year-2) %>%
...
f3.1.1_txt <- filter(inc_regional, year>=report_year-2 & entity == "African Region") %>%
  mutate(c_newinc_p = lag(c_newinc)) %>%
  mutate(pct_dif = (c_newinc - c_newinc_p)*100/c_newinc_p) %>%
  filter(year==report_year-1) %>%
  select(pct_diff_AFR = pct_dif) %>%
  cbind(f3.1.1_txt)
...
agesex_global <- agesex %>%
  summarise(across(newrel_m014:e_pop_f65, sum, na.rm = TRUE)) %>%
...
agesex_agg_pop <- agesex_pop %>%
  summarise(across(e_pop_m014:e_pop_f65, sum, na.rm = TRUE)) %>%
  mutate(group_name = "global")
...
agesex_agg_pop <- agesex_pop %>%
  ...
  # flip to long format
  pivot_longer(cols = e_pop_m014:e_pop_f65,
               names_to = c("sex", "age_group"),
...
f3.1.5_txt <- agesex_agg_notifs_wide %>%
  rowwise() %>%
  mutate(men = sum(across(starts_with("newrel_m"))) - newrel_m014,
         women = sum(across(starts_with("newrel_f"))) - newrel_f014,
